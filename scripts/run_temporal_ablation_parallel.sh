#!/usr/bin/env bash
# Run the strict baseline vs DGTE ablation with one policy server per job.
# The protocol is identical to run_temporal_ablation.sh; this launcher only
# places the eight independent jobs on separate GPUs and ports.
set -euo pipefail
...
SERVER_PORT_BASE=${SERVER_PORT_BASE:-8100}
GPU_IDS=${GPU_IDS:-"0 1 2 3 4 5 6 7"}
RUN_ID=${RUN_ID:-dgte_ablation_parallel_$(date +%Y%m%d_%H%M)}
...
declare -a CONTROLLERS=(baseline dgte)
job_index=0
for controller in "${CONTROLLERS[@]}"; do
    for suite in "${SUITES[@]}"; do
        port=$((SERVER_PORT_BASE + job_index))
        start_server "${GPUS[$job_index]}" "$port" "$EXP_DIR/logs/server_${controller}_${suite}.log"
...
for index in "${!SERVER_PIDS[@]}"; do
    ...
    log="$EXP_DIR/logs/server_${CONTROLLERS[$((index / 4))]}_${SUITES[$((index % 4))]}.log"
    ready=0
    ...
    if [[ "$ready" -ne 1 ]]; then
        echo "Policy server on port $port failed to start; see $log" >&2
        exit 1
    fi
done
...
for controller in "${CONTROLLERS[@]}"; do
    for suite in "${SUITES[@]}"; do
        run_job "$controller" "$suite" "$((SERVER_PORT_BASE + job_index))"
        job_index=$((job_index + 1))
    done
done
